"""Render the production welcome screen on the 128x64 display."""
...
def massage_sprite_as_gif(
    display_width: int, display_height: int, offset_x: int, offset_y: int = 0
) -> bytes:
    """Create one full-screen frame with the chair shifted for vibration."""
    image = Image.new("1", (display_width, display_height), 0)
    draw_massage_chair(image, display_width - 21 + offset_x, 27 + offset_y)
    output = BytesIO()
...
def main() -> int:
    ...
    if platform.machine() == "armv6l":
        yocto_api.YAPI.SelectArchitecture(os.getenv("YOCTO_ARCHITECTURE", "armhf"))
