...
def configure_pi_zero_w_architecture():
    """Use the correct Yoctopuce USB library on 32-bit Pi Zero W systems."""
    if platform.machine() == "armv6l":
        architecture = os.getenv("YOCTO_ARCHITECTURE", "armhf")
        yocto_api.YAPI.SelectArchitecture(architecture)
        print(f"Yoctopuce architecture selected: {architecture}")
...
def main():
    args = parse_args()
    configure_pi_zero_w_architecture()
    ...
    events = SimpleQueue()
    # Render before initiating MQTT. A missing network or broker must never
    # leave the local OLED blank.
